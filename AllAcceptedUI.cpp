...
//NOTE: Not subclassed as all functions edited anyways
#include <fstream>

#include "AllAcceptedUI.h"
...
//////////////////////////////////////////////////////////////////////////
// Default constructor
AllAcceptedUI::AllAcceptedUI(Manager* aManager) 
	: aTable(3, 3, false),
	aLabel("List of all accepted applications sorted by Major GPA and Research Area"),
	backButton("Back"),
	cancelButton("Cancel"),
	saveButton("Save"),
	summary(10, false, Gtk::SELECTION_NONE)
{
	...
	backButton.signal_clicked().connect(
			sigc::bind<Glib::ustring>( sigc::mem_fun(*this, &AllAcceptedUI::on_backButton), "Back") );
	cancelButton.signal_clicked().connect(
			sigc::bind<Glib::ustring>( sigc::mem_fun(*this, &AllAcceptedUI::on_cancelButton), "Cancel") );
	saveButton.signal_clicked().connect(
			sigc::bind<Glib::ustring>( sigc::mem_fun(*this, &AllAcceptedUI::on_saveButton), "Save") );

	show_all_children();

	//cout << "CONSTRUCT AllAcceptedUI" << endl;
}

//////////////////////////////////////////////////////////////////////////
// Destructor
AllAcceptedUI::~AllAcceptedUI() {
	//cout << "DESTRUCT AllAcceptedUI" << endl;
}

//////////////////////////////////////////////////////////////////////////
// Event handler for the back button
void AllAcceptedUI::on_backButton(const Glib::ustring& data){
...
void AllAcceptedUI::on_cancelButton(const Glib::ustring& data){
	AdminOptionUI* adminWin = new AdminOptionUI(manager);
	adminWin->show();
	delete this;
}
...
void AllAcceptedUI::on_saveButton(const Glib::ustring& data){

	string cn = "./AppSummaries/AllAcceptedSummary";
	int i = 0;
	...
	outFile << "*NEW SUMMARY OF ALL ACCEPTED APPLICATIONS*" << endl;
...
void AllAcceptedUI::fillSummary() {
	int i = 0;
	...
	while(currCrs != 0){
		...
		if (copyU->front() != 0) {
			...
			while (currUNode != 0) {
				if(currUNode->data->getStatus()==ACCEPTED){
					summary.append("");   
		...
		if (copyG->front() != 0) { 
			...
			while (currGNode != 0) {
				if(currGNode->data->getStatus()==ACCEPTED){
					summary.append("");
