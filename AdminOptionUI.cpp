	...
	backButton("Back")
{
	...
	adminCombo.append("View summary of assigned applications");
...
void AdminOptionUI::on_combo_change(){
	...
	if(adminCombo.get_active_text() == "View summary of pending applications")
		bCombo.show();
	else if(adminCombo.get_active_text() == "View summary of assigned applications")
		bCombo.hide();
	else bCombo.hide();
...
void AdminOptionUI::on_nextButton(const Glib::ustring& data){
	if(bCombo.get_active_text() == "For one course, sorted by Major GPA and Research Area"){
		...
		AllAcceptedUI * allAccepted = new AllAcceptedUI(manager);
		allAccepted->show();
		delete this;
	}
}
